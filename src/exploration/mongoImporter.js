  ...
  MongoClient.connect(
    ...
    async (err, client) => {
      ...
      await Promise.all([
        addCollectionToDb(
          db,
          'skillsfuture_all_courses',
          await readFromFile(SKILLSFUTURE_ALL_COURSES_DUMP_FILE),
        ),
        addCollectionToDb(
          db,
          'skillsfuture_individual_courses',
          await readFromFile(SKILLSFUTURE_INDIVIDUAL_COURSES_DUMP_FILE),
        ),
        addCollectionToDb(
          db,
          'coursera_all_courses',
          await readFromFile(COURSERA_ALL_COURSES_DUMP_FILE),
        ),
        addCollectionToDb(
          db,
          'coursera_individual_courses',
          await readFromFile(COURSERA_INDIVIDUAL_COURSES_DUMP_FILE),
        ),
      ]);
