...
const getCourseraCoursesFromSkillsfutureIndividualCourses = individualCourses =>
  individualCourses
    .filter(course => course.data)
    .filter(
      course => course.data.trainingProviderAlias.toLowerCase() === 'coursera',
    )
...
const getCourseraIndividualCourses = courseraCourses =>
  courseraCourses
...
const generateSpecializationCoursesField = (
  ...
  courseIds.map(courseraCourseId => {
    const course = getCourseraCourseFromCourseIdMap(
      courseraCourseId,
      courseraCoursesIdMap,
    );
    ...
    if (courseraCourseSlug in courseraSkillsfutureCourseMap) {
      const skillsfutureCourse =
        courseraSkillsfutureCourseMap[courseraCourseSlug];
...
const getCourseraPartnersMap = courseraPartners =>
  courseraPartners.reduce((partnersMap, partner) => {
    const partnerId = partner.id;
    return { ...partnersMap, [partnerId]: partner };
  }, {});

const addPartnerNamesToPartnerIds = (partnerIds, courseraPartnersMap) =>
  partnerIds.map(partnerId => ({
    id: partnerId,
    name: courseraPartnersMap[partnerId].name,
  }));

const addPercentageCoursesCoveredBySkillsfuture = courses => {
  const numberOfCourses = courses.length;
  const numberOfCoursesCoveredBySkillsfuture = courses.filter(
    ...
    .map(specialization => {
      try {
        const newSpecializationObject = {
          ...
          partnerIds: addPartnerNamesToPartnerIds(
            specialization.partnerIds,
            courseraPartnersMap,
          ),
        };

        newSpecializationObject.percentageCoveredBySkillsfuture = addPercentageCoursesCoveredBySkillsfuture(
          newSpecializationObject.courses,
        );
        ...
        logger.warn(
          `error parsing specialization: id=${specialization.id}, slug=${
            specialization.slug
          }`,
        );
        return undefined;
...
const generateSkillsfutureCourseraStore = async (
  ...
  const courseraCoursesIdMap = getCourseraIndividualCourses(
    courseraIndividualCourses,
  );
  const courseraSkillsfutureCourseMap = getCourseraCoursesFromSkillsfutureIndividualCourses(
    skillsfutureIndividualCourses,
  );
  const courseraPartnersMap = getCourseraPartnersMap(courseraPartners);

  const mergedSpecializationsMatrix = generateMergedMatrix(
    courseraSpecializations,
    courseraCoursesIdMap,
    courseraSkillsfutureCourseMap,
    courseraPartnersMap,
  ...
  const store = {
    specializations: mergedSpecializationsMatrix,
    informationScrapeTimestamp,
  };
